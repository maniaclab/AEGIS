...
/** Describe the JSON-RPC payload of an MCP request, e.g. `mcp=tools/call tool=get_ticket`. */
...
export function requestLogger(req: Request, res: Response, next: NextFunction): void {
    ...
    res.on('finish', () => {
        ...
        const line = `req#${id} <-- ${res.statusCode} ${ms.toFixed(1)}ms`;
...
/** Log an outbound call to the GGUS API. */
export function logUpstream(method: string, path: string, status: number, startedAt: bigint): void {
    const ms = Number(process.hrtime.bigint() - startedAt) / 1e6;
    const line = `ggus ${method} ${path} ${status} ${ms.toFixed(1)}ms`;
